...
* LRU/LFU/FIFO算法剔除
  ...
  * 维护成本: 低
* 超时删除
  * 使用场景: 设置过期时间，业务可以容忍一段时间内，缓存层数据和存储层数据不一致
  * 一致性: 在过期时间内有一致性问题
  * 维护成本: 低
* 主动更新
  * 使用场景: 应用方对于数据的一致性要求高
...
* 解决方法
  * 缓存空对象
    ...
    * 会有一个不一致的窗口期
  * 布隆过滤器
    * 适用于数据命中不高，数据相对固定、实时性低的场景
    * 代码维护成本较高

## 11.5 无底洞优化
* 客户端一次批量操作会涉及多次网络操作，随着批量操作随着节点增多，耗时会不断加大
...
## 11.6 雪崩优化(stamping herd)
* 保证缓存层服务的高可用性
* 依赖隔离组件为后端限流并降级
  * 对重要资源都进行隔离，让每种资源都单独运行在自己的线程池中，互不影响
  * Hystrix
* 提前演练
